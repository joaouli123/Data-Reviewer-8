                              ...
                              <div className="mt-2 border border-slate-200 rounded-lg overflow-hidden shadow-sm">
                                <div className="bg-gradient-to-r from-slate-100 to-slate-50 px-2.5 py-1.5 border-b border-slate-200">
                                  <span className="text-[10px] font-bold text-slate-700 uppercase tracking-wide">
                                    📋 Histórico de Recebimentos ({paymentHistory.length})
                                  </span>
                                </div>
                                <div className="divide-y divide-slate-100 bg-white">
                                  {paymentHistory.map((entry, hIdx) => (
                                    <div key={`${installment.id}-payment-${hIdx}`} className="flex items-center justify-between px-2.5 py-2 hover:bg-emerald-50/50 transition-colors">
                                      <div className="flex items-center gap-2.5">
                                        <span className="flex items-center justify-center w-5 h-5 rounded-full bg-emerald-100 text-emerald-700 text-[9px] font-bold">
                                          {hIdx + 1}
                                        </span>
